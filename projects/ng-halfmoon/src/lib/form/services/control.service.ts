...
export class ControlService implements OnDestroy {
  ...
  private _currentStatus$: Subject<ControlStatus> =
    new Subject<ControlStatus>();
  ...
  init(control: FormControl): void {
    ...
    this.subscription = this.currentControl.statusChanges
  ...
  triggerStatusChange() {
    if (
      ...
      [ControlStatus.INVALID, ControlStatus.VALID].includes(
        this.currentControl.status as ControlStatus
      )
    ) {
